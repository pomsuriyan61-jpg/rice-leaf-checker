...
(function (global) {
  ...
  var CSS = [
    ...
    '.fc-notice{font-size:13px;line-height:1.7;color:#6b6b6b;background:#f7f7f5;border-radius:8px;padding:12px 14px;margin-bottom:16px}'
  ].join('');
  ...
  // ------------------------------------------------------------
  // 4b. ปุ๋ยคอก — ผู้ใช้กรอกอัตราเอง
  // ------------------------------------------------------------
  //
  // จงใจไม่ใส่อัตราตั้งต้นให้ ตามเหตุผลเดิมใน fertilizer-guide.js
  // ที่ว่ามูลไก่เข้มข้นกว่ามูลวัวควายหลายเท่า การให้เลขกลางๆ กับทุกคน
  // คือการเดาแทนเกษตรกร ซึ่งเป็นความเสียหายที่ป้องกันได้ด้วยการไม่เดา
  //
  // แต่การล็อกไม่ให้กรอกเลยก็ปิดประตูใส่คนที่ปรึกษาหมอดินมาแล้ว
  // และมีตัวเลขของแปลงตัวเองอยู่ในมือ จึงเปลี่ยนเป็นให้กรอกอัตราเอง
  // ระบบทำหน้าที่คูณเลขและแปลงหน่วยให้ ไม่ใช่ตัดสินใจแทน
  function renderManure(container, defaults) {
    if (!container) return;
    defaults = defaults || {};
    var uid = 'fm' + Math.random().toString(36).slice(2, 8);

    container.innerHTML =
      '<div class="fc-wrap">' +
        '<div class="fc-notice">' +
          'ระบบไม่กำหนดอัตราให้ เพราะปริมาณที่เหมาะสมต่างกันมากตามชนิดสัตว์และความอุดมสมบูรณ์ของดิน ' +
          'กรอกอัตราที่ได้จากหมอดินอาสาประจำตำบลหรือที่ใช้อยู่เดิม แล้วระบบจะคำนวณปริมาณรวมให้' +
        '</div>' +
        '<div class="fc-grid">' +
          fieldHtml('พื้นที่ (ไร่)',
            '<input type="number" id="' + uid + '-rai" class="fc-input" min="0.5" step="0.5" value="' +
            esc(defaults.rai != null ? defaults.rai : 1) + '">') +
          fieldHtml('อัตราที่จะใช้ (กก./ไร่)',
            '<input type="number" id="' + uid + '-rate" class="fc-input" min="1" step="50" placeholder="เช่น 1000" value="' +
            esc(defaults.manureRate != null ? defaults.manureRate : '') + '">') +
        '</div>' +
        '<div class="fc-error" id="' + uid + '-err" hidden></div>' +
        '<div id="' + uid + '-out"></div>' +
      '</div>';
    ...
    var raiInput = container.querySelector('#' + uid + '-rai');
    var rateInput = container.querySelector('#' + uid + '-rate');
    var errBox = container.querySelector('#' + uid + '-err');
    var outBox = container.querySelector('#' + uid + '-out');
    ...
    function update() {
      ...
      // ช่องอัตราว่างไม่ถือเป็นความผิดพลาด เพราะผู้ใช้อาจยังไม่รู้ตัวเลข
      // แค่ยังคำนวณไม่ได้เท่านั้น จึงไม่ขึ้นข้อความสีแดงให้ตกใจ
      if (!isFinite(rate) || rate <= 0) {
        errBox.hidden = true;
        outBox.innerHTML = '<div class="fc-hint">กรอกอัตราต่อไร่เพื่อดูปริมาณรวม</div>';
        return;
      }
      errBox.hidden = true;

      var totalKg = rai * rate;
      var rows = [
        rowHtml('อัตราที่กรอก', fmt(rate) + ' กก./ไร่'),
        rowHtml('พื้นที่', rai + ' ไร่'),
        rowHtml('รวมทั้งแปลง', fmt(totalKg) + ' กก.')
      ];

      // ปุ๋ยคอกใช้ปริมาณมากจนหน่วยกิโลกรัมอ่านยาก และการขนย้ายคิดเป็นตัน
      // จึงแสดงหน่วยตันควบคู่เมื่อถึงระดับที่มีนัยสำคัญ
      if (totalKg >= 1000) {
        rows.push(rowHtml('คิดเป็น', (totalKg / 1000).toFixed(1) + ' ตัน'));
      }

      outBox.innerHTML = cardHtml('ปริมาณที่ต้องเตรียม', 'ปุ๋ยคอก',
        'ต้องเป็นมูลที่ผ่านการหมักจนสลายตัวดีแล้ว ใส่ช่วงเตรียมดินแล้วไถกลบขณะดินมีความชื้นพอเพียง', rows);
    }

    [raiInput, rateInput].forEach(function (node) {
      node.addEventListener('input', update);
    });
    update();
  }
  ...
  // แยกว่ากล่องว่างที่เจอเป็นของปุ๋ยชนิดไหน เพราะทั้งสองหน้าตาเหมือนกัน
  // แต่ต้องใช้ฟอร์มคนละแบบ ปุ๋ยเคมีมีอัตราทางการให้ ส่วนปุ๋ยคอกต้องให้กรอกเอง
  function detectFertType() {
    var text = document.body.textContent || '';
    if (text.indexOf('มูลสัตว์สด') !== -1) return 'manure';
    if (text.indexOf('ระยะกำเนิดช่อดอก') !== -1 || text.indexOf('16-20-0') !== -1) return 'chem';
    return null;
  }
  ...
  function tryMount() {
    ...
    var fertType = detectFertType();
    if (!fertType) return;

    box.dataset[MOUNTED_FLAG] = '1';
    try {
      if (fertType === 'manure') {
        renderManure(box, global.FC_DEFAULTS || {});
      } else {
  ...
  global.FertCalc = { render: renderChem, renderManure: renderManure, data: CHEM_CALC };
})(window);
